...
guard::Observation sample(std::uint64_t raw_ns,
...
TEST(SessionClockGuardCatkin, Sha256AndStrictSimulationPolicy) {
  EXPECT_EQ("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad",
            guard::sha256Hex("abc"));
  const auto config =
      guard::FrozenConfigLoader::parse(simulationConfig(), std::string(64U, 'd'));
...
TEST(SessionClockGuardCatkin, IdentityRollbackNeedsStrictlyNewerEpoch) {
  auto config =
      guard::FrozenConfigLoader::parse(simulationConfig(), std::string(64U, 'd'));
  guard::SessionClockGuard clock_guard(std::move(config), 40U,
...
TEST(SessionClockGuardCatkin, ModeMismatchAndImpossibleUncertaintyFailClosed) {
  ...
  wrong_mapping.replace(mapping, std::string("session_clock.mapping=identity").size(),
  ...
  const auto uncertainty = impossible.find(
      "route.px4-01.source_uncertainty_ns=6000000");
  impossible.replace(
      uncertainty,
