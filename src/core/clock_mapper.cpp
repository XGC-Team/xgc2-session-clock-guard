...
ClockMapper::ClockMapper(const Route &route, const ThresholdPolicy &policy,
...
bool ClockMapper::initializeEpoch(std::uint64_t epoch,
  ...
  if (epoch == 0U || epoch <= status_.epoch || invalid_session_start ||
    ...
    if (reason != nullptr) {
      *reason = "epoch must be strictly newer, monotonic start must be "
                "nonzero, and affine Session start must be nonzero";
    }
    return false;
  }
  samples_.clear();
...
void ClockMapper::poll(std::uint64_t monotonic_now_ns) {
  ...
  for (const auto &timeline : timelines_) {
    ...
    const std::uint64_t age_limit = timeline.seen
                                        ? policy_.max_sample_age_ns
                                        : policy_.startup_lock_timeout_ns;
    if (monotonic_now_ns - reference > age_limit) {
      recordFailure(timeline.seen
                        ? "VRPN stream silence exceeds frozen maximum age"
                        : "required VRPN stream did not start before the "
                          "frozen startup lock timeout",
                    !timeline.seen);
      return;
    }
  }
...
ClockMapper::Estimate
ClockMapper::estimateAffine(const std::deque<Sample> &samples,
  ...
  const long double raw_anchor =
      static_cast<long double>(samples.front().raw_ns);
  ...
  result.uncertainty_ns =
      saturatingAdd(route_.source_uncertainty_ns, result.jitter_ns);
...
ClockMapper::Estimate
ClockMapper::estimateIdentity(std::uint64_t raw_ns) const {
  Estimate result;
...
void ClockMapper::recordFailure(const std::string &reason,
                                bool immediate_loss) {
  if (status_.state == GuardState::Lost) {
    return;
  }
  if (status_.consecutive_failures <
  ...
  recovery_samples_ = 0U;
  if (immediate_loss ||
      status_.consecutive_failures >= policy_.lost_after_failures) {
    status_.state = GuardState::Lost;
    status_.reason =
        reason +
        "; a new Core-started process with a strictly newer epoch is required";
    return;
  }
...
void ClockMapper::authorityFailure(const std::string &reason,
                                   bool immediate_loss) {
...
TimestampEnvelope ClockMapper::makeEnvelope(const Observation &observation,
  ...
  envelope.mapped_session_stamp_ns =
      accepted ? status_.last_session_stamp_ns : 0U;
  ...
  envelope.canonical_publish_allowed =
      accepted && status_.state == GuardState::Locked;
  envelope.reason = reason;
  return envelope;
}

TimestampEnvelope
ClockMapper::rejectedEnvelope(const Observation &observation,
                              const std::string &reason) const {
...
TimestampEnvelope ClockMapper::observe(const Observation &observation) {
  if (observation.slot != route_.slot ||
    ...
    return reject(observation, "observation does not match its frozen route",
                  true);
  }
  if (status_.epoch == 0U) {
  ...
  if (status_.state == GuardState::Lost) {
    return makeEnvelope(observation, false,
                        "clock guard is lost; a new Core-started process with "
                        "a strictly newer epoch is required");
  }
  ...
  for (const auto &required_timeline : timelines_) {
    if (!required_timeline.seen && observation.receive_monotonic_stamp_ns -
                                           epoch_start_monotonic_stamp_ns_ >
                                       policy_.startup_lock_timeout_ns) {
      return reject(observation,
                    "required VRPN stream did not start before the frozen "
  ...
  if (timeline.seen &&
    ...
    return reject(observation,
                  "steady receive timestamp repeated or moved backwards", true);
  }
  ...
  if (prior_affine_estimator_ready && have_estimate_) {
    ...
    const std::int64_t prediction_error =
        observation.receive_session_stamp_ns >= prediction.mapped_session_ns
            ? static_cast<std::int64_t>(std::min<std::uint64_t>(
                  observation.receive_session_stamp_ns -
                      prediction.mapped_session_ns,
    ...
    if (absoluteUnsigned(prediction_error) > policy_.max_sample_age_ns) {
      return reject(observation,
                    "sample is stale or implausibly ahead of Session time",
                    false);
    }
  }
  ...
  if (mapping_ == ClockMapping::Identity) {
    ...
    const bool raw_stamp_already_fitted = std::any_of(
        candidate.begin(), candidate.end(), [&](const Sample &sample) {
          return sample.raw_ns == observation.raw_source_stamp_ns;
        });
    if (!raw_stamp_already_fitted) {
      candidate.push_back({observation.raw_source_stamp_ns,
                           observation.receive_session_stamp_ns});
    }
  ...
  if (mapping_ == ClockMapping::AffineToSession && estimator_quality_ready) {
    if (have_estimate_ &&
  ...
  status_.last_receive_monotonic_stamp_ns =
      observation.receive_monotonic_stamp_ns;
