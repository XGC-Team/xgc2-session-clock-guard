...
std::string sha256Hex(const std::string& bytes) {
  ...
  for (std::size_t chunk = 0; chunk < data.size(); chunk += 64U) {
    ...
    for (std::size_t i = 0; i < words.size(); ++i) {
      const std::uint32_t upper_e = rotateRight(e, 6U) ^ rotateRight(e, 11U) ^
                                    rotateRight(e, 25U);
      const std::uint32_t choose = (e & f) ^ ((~e) & g);
      const std::uint32_t temp1 = h + upper_e + choose + kRoundConstants[i] + words[i];
      const std::uint32_t upper_a = rotateRight(a, 2U) ^ rotateRight(a, 13U) ^
                                    rotateRight(a, 22U);
