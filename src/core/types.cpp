...
SourceDomain parseSourceDomain(const std::string& text) {
  ...
  throw std::invalid_argument("source_domain must be exactly simulation or physical");
}
...
SessionTimeAuthority parseSessionTimeAuthority(const std::string& text) {
  ...
  throw std::invalid_argument(
      "session_clock.authority must be exactly gazebo-simulation or station-wall-monotonic");
}
...
std::uint64_t parseRosPrivateEpochId(const std::string& text) {
  if (!text.empty() && (text.front() == '"' || text.back() == '"')) {
    if (text.size() < 3U || text.front() != '"' || text.back() != '"') {
      throw std::invalid_argument(
          "ROS private epoch must use exactly one complete ASCII double-quote wrapper");
    }
...
std::string rawRoot(SourceDomain domain) {
  return domain == SourceDomain::Simulation ? kSimulationRawRoot : kPhysicalRawRoot;
}
